...
void execute_add_test(void) {
    ...
    //printf("DRIVER_DEBUG: result=%d, expected='%s'\n", result, g_expected_result);
...
void execute_sort_test(void) {
    ...
    // Initialize test data (using underscores to match)
    safe_strncpy(names[0], "Pizza_Margherita", MAX_NAME_LENGTH);
    safe_strncpy(categories[0], "Main", MAX_CATEGORY_LENGTH);
    prices[0] = 12.99;
    ...
    safe_strncpy(names[3], "Iced_Tea", MAX_NAME_LENGTH);
    safe_strncpy(categories[3], "Beverage", MAX_CATEGORY_LENGTH);
    prices[3] = 2.99;
    
    safe_strncpy(names[4], "Grilled_Chicken", MAX_NAME_LENGTH);
    safe_strncpy(categories[4], "Main", MAX_CATEGORY_LENGTH);
    prices[4] = 15.99;
